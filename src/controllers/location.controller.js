const { location: locationModel } = require("../models/index.js");
...
const getDataTable = async (req, res) => {
  ...
  const location = await locationModel.findAndCountAll({
    where: whereClause,
    limit,
    offset,
  });

  const pages = Math.ceil(location.count / limit);
...
const getDataById = async (req, res) => {
  ...
  const findData = await locationModel.findOne({
    where: { uuid },
  });
...
const createData = async (req, res) => {
  ...
  const newData = await locationModel.create({
    name,
    sequence,
    code,
    is_active: true,
