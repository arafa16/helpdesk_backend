...
const getDatas = async (req, res) => {
  ...
  const rows = await ticketTroubleCategoryModel.findAll({
    where,
    order,
  });
...
const getDataTable = async (req, res) => {
  ...
  const data = await ticketTroubleCategoryModel.findAndCountAll({
    where: whereClause,
    limit,
    offset,
  });
  ...
  return res.status(200).json({
    success: true,
    message: "Get ticket trouble categories successfully",
    data: data.rows,
    meta: {
      total: data.count,
      page,
...
const getDataById = async (req, res) => {
  ...
  const findData = await ticketTroubleCategoryModel.findOne({
    where: { uuid },
  });
...
const createData = async (req, res) => {
  ...
  const newData = await ticketTroubleCategoryModel.create({
    name,
    sequence,
    code,
    is_active: true,
