const { user_status: userStatusModel } = require("../models");
...
const getDatas = async (req, res) => {
  ...
  const { count, rows } = await userStatusModel.findAndCountAll({
    where,
    limit,
    offset,
    order,
...
const getDataById = async (req, res) => {
  ...
  const findData = await userStatusModel.findOne({
    where: { uuid },
  });
...
const createData = async (req, res) => {
  ...
  const newData = await userStatusModel.create({
    name,
    sequence,
    code,
    is_active: true,
