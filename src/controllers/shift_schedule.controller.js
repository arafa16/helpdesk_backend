const { shift_schedule: shiftScheduleModel } = require("../models/index.js");
...
const getDatas = async (req, res) => {
  ...
  const { count, rows } = await shiftScheduleModel.findAndCountAll({
    where,
    limit,
    offset,
    order,
...
const getDataById = async (req, res) => {
  ...
  const findData = await shiftScheduleModel.findOne({
    where: { uuid },
  });
...
const createData = async (req, res) => {
  ...
  const newData = await shiftScheduleModel.create({
    name,
    start_date,
    end_date,
    is_active: true,
